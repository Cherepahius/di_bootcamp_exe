document.addEventListener("DOMContentLoaded", () => {
    ...
    // Check if robots container is present
    const robotContainer = document.getElementById('robots');
    console.log("Robot container:", robotContainer);
    if (!robotContainer) {
    ...
    const displayRobots = (robotsToDisplay) => {
        ...
        robotsToDisplay.forEach(robot => {
            ...
            robotCard.innerHTML = `
                ...
                <p>${robot.email}</p>
            `;
            console.log("Robot card:", robotCard);
    ...
    searchInput.addEventListener('input', (event) => {
        ...
        const filteredRobots = robots.filter(robot => robot.name.toLowerCase().includes(searchQuery));
